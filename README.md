...
проверяет наличие polygon.json:

если файл есть и в нём есть объекты с непустым kadastr, читает их и работает в режиме обновления polygon.json;

иначе переходит в режим cad_list.txt → result.json;
...
получает sidebar_shadow через get_sidebar_shadow(driver);

вызывает search_and_open_card(driver, sidebar_shadow, cad) и добавляет результат в список;

пишет в консоль подробный лог шагов;

в режиме polygon.json:

сопоставляет полученные URL по кадастровым номерам;

перезаписывает поля kadastrurl в polygon.json и сохраняет файл;

в режиме cad_list.txt:

записывает список результатов в result.json;
...
Сделать более гибкую обработку ошибок и логирование (отдельно отмечать «участок не найден», сетевые ошибки и изменения DOM).

Вынести переключатель тихого режима, таймауты и параметры Chrome в конфигурационный файл.

Опционально добавлять к share‑ссылкам дополнительные параметры (active_layers, преднастройки слоёв и т.п.).
